...
This will pickle the results of the test into a local temp file so that the web
page itself can load the data very quickly and not rely on running the script
live to load. 

IMPORTANT!!!
Set this script up to be run in the crontab of the Linux server host!
"""
import datetime
import requests
import pickle
...
def pickle_multiple_vars(start_time, maindata):
    """ Function to pickle out the data points to be read in by the website."""
    with open('data.tmp', 'wb') as pfile:
        pickle.dump([start_time, maindata], pfile)
        print('Pickled data')
...
def determine_health_all_sites():
    ...
    print(my_return, start_time)
    pickle_multiple_vars(start_time, my_return)
    return my_return
...
def make_airport_list():
    """ Function to get all sites, then parse down to only airports"""
